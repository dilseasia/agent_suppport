...
from app.services.tools import weather, dealership, appointment
from app.Agent.classifier import classify_query_with_gemini
from app.Agent.support.tool import SupportAgent   # 👈 Import support agent
from app.Agent.support.schema import SupportRequest  # 👈 Schema import
...
# Available agents
available_agents = {
    "support": support_agent.handle_request,   # 👈 Added support agent
    "appointment": appointment.handle_appointment,
    "general": None
}

def get_system_message_for_intent(intent: str) -> str:
    system_messages = {
        "greeting": "You are Lex, a friendly sales assistant for SuperCar dealerships.",
        "support": "You are Lex, a helpful support agent for SuperCar dealerships. Help with appointments, dealership, weather, or general queries.",
        "appointment": "You are Lex, a scheduling specialist for SuperCar dealerships.",
        "estimate": "You are Lex, a pricing specialist for SuperCar dealerships.",
        "information": "You are Lex, an information specialist for SuperCar dealerships.",
        "general": "You are Lex, a knowledgeable sales assistant for SuperCar dealerships."
    }
    return system_messages.get(intent, system_messages["general"])
...
def healthcheck():
    return {
        "status": "OK",
        "message": "SuperCar Virtual Sales Assistant API is running!"
    }
...
async def process_query(request: QueryRequest):
    ...
    conversation_history.append({
        "role": "user",
        "content": f"[Intent: {primary_intent}] {corrected_query}"
    })
    session_storage[request.session_id] = conversation_history
        ...
        try:
            ...
            if out_of_context:
                msg = (
                    ...
                    "I can help with cars, service, appointments or dealership queries. "
                    "Could you rephrase your question?"
                )
                yield {"event": "chunk", "data": msg}
                return

            # Pick best agent
            highest_intent = max(intent_scores, key=intent_scores.get, default="general")
            agent_function = available_agents.get(highest_intent)

            if agent_function:
                if highest_intent == "support":
                    #  Wrap request into SupportRequest
                    support_req = SupportRequest(message=request.query)
                    support_response = agent_function(support_req)  # returns SupportResponse
                    yield {"event": "chunk", "data": support_response.response}
                else:
                    response = agent_function(request.query)
                    yield {"event": "chunk", "data": response}
            else:
                yield {"event": "chunk", "data": "I'm not sure how to handle that, could you clarify?"}

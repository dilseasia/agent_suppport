...
class EventsListRequest(BaseModel):
    """
    Request schema for listing Google Calendar events.
    
    Retrieves a list of events from the specified Google Calendar.
    All datetime fields should be in ISO 8601 format.
    """
    ...
    order_by: Optional[OrderBy] = Field(
        ...
        example=OrderBy.START_TIME
    )
    show_deleted: bool = Field(
        False,
        description="Include deleted/cancelled events",
        example=False
    )
    page_token: Optional[str] = Field(
        None,
        description="Token for pagination",
        example="Cg0KC2V4YW1wbGVfdG9rZW4="
    )
...
@router.post("/events/list", tags=["Events"])
async def list_events(request: EventsListRequest):
    ...
    **Returns:**
    ...
    if request.order_by:
        arguments["order_by"] = request.order_by.value
...
class FindEventRequest(BaseModel):
    ...
    calendar_id: str = Field("primary", description="Calendar ID to search in")
    time_min: Optional[str] = Field(None, description="Start of time range (ISO 8601)")
    time_max: Optional[str] = Field(None, description="End of time range (ISO 8601)")
    max_results: int = Field(10, description="Maximum number of results", ge=1, le=2500)

@router.post("/event/find", tags=["Events"])
async def find_event(request: FindEventRequest):
    """
    Find events in Google Calendar using a search query.

    Searches through event titles, descriptions, locations, and attendees.
    Optional time filters can be applied.
...
def get_event(calendar_id: str = Query("primary"), event_id: str = Query(...)):
    # Use camelCase key for Composio
    payload = {"calendarId": calendar_id}

    result = execute_composio_tool("GOOGLECALENDAR_EVENTS_LIST", payload)
    ...
    # Extract all events
    all_events = result.get("data", {}).get("items", [])

    # Events might be a dict with single event
